 ...
 */
@Slf4j
public  class CmmnServiceImpl<JPA, MAPPER, DOMAIN, IDTYPE> implements CmmnService<DOMAIN, IDTYPE> {
	/**
	 * jpa
	 */
	 ...
	 * mybatis mapper
	 */
	private CmmnMapper<DOMAIN, IDTYPE> mapper;
	/**
	 * domain
	 */
	...
	public void deleteByBizKey(DOMAIN domain) {
		...
		try {
			...
			delete((IDTYPE)((CmmnDomain)domain2).getId());
	...
	public DOMAIN findByBizKey(DOMAIN domain) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, SecurityException {
		...
		if(CmmnUtils.isEmpty(bizFields)) {
	...
	public DOMAIN update(IDTYPE id, DOMAIN domain) {
		...
		CmmnUtils.getFieldNames(domain).forEach(fieldName->{
			...
			//
			Object value = CmmnUtils.getFieldValue(domain, fieldName);
			CmmnUtils.setFieldValue(domain2, fieldName, value);
		});
	...
	public void updateAll(Iterable<DOMAIN> domains) {
		...
		domains.forEach(x->{
			...
			//
			update((IDTYPE)((CmmnDomain)x).getId(), x);
		});
	}
...
//		}
	...
	private List<FieldAndOrder> getBizFields(DOMAIN domain){
		...
		CmmnUtils.getFieldsUpTo(domain.getClass(), fields);
		...
		fields.forEach(f->{
			...
			//
			CmmnField dsField = f.getAnnotation(CmmnField.class);
	...
	private String getMethodName(String pre, List<FieldAndOrder> fields) {
		...
		for(FieldAndOrder e : fields) {
			...
			str += CmmnUtils.capitalize(e.field.getName());
		}
	...
	protected  void set(JPA j, MAPPER m, DOMAIN d) {
		...
		this.mapper = (CmmnMapper<DOMAIN, IDTYPE>) m;

 ...
 */
@Slf4j
public  class AnalsServiceImpl<JPA, MAPPER, DOMAIN, IDTYPE> implements AnalsService<DOMAIN, IDTYPE> {
	/**
	 * jpa
	 */
	...
	private AnalsMapper<DOMAIN, IDTYPE> mapper;
	/**
	 * domain
	 */
	@SuppressWarnings("unused")
	...
	public DOMAIN update(IDTYPE id, DOMAIN domain) {
		...
		AnalsUtils.getFieldNames(domain).forEach(x->{
			if("id".equals(x)) {
				return;
			}
			
			//
			Object value = AnalsUtils.getFieldValue(domain, x);
			AnalsUtils.setFieldValue(domain2, x, value);
		});
	...
	private boolean existsBizKey(DOMAIN domain){
		...
		//
		for(Field f : fields) {
			AnalsField analsField = f.getAnnotation(AnalsField.class);

			//
			if(null != analsField && analsField.bizKey()) {
				return true;
			}
		}
	...
	private String getMethodName(String pre, List<FieldAndOrder> fields) {
		...
		for(FieldAndOrder e : fields) {
			...
			if(AnalsUtils.isNotEmpty(str)) {
				str += "And";
			}
			str += AnalsUtils.capitalize(e.field.getName());
		}
...
//		log.debug("<<.getParamTypes - {}", classes);
	...
	protected  void set(JPA j, MAPPER m, DOMAIN d) {
		...
		this.mapper = (AnalsMapper<DOMAIN, IDTYPE>) m;

...
[[resources-design-layer-groups-list]]
=== 디자인 레이어 그룹 목록 조회
...
`GET` point 정보외에 추가적으로 buffer 를 설정하여 검색 반경을 설정할 수 있다.
...
`GET` point 와 마찬가지로 추가적으로 buffer 설정이 가능하며, 시작 지점과 종료 지점이 같지 않을 경우 lineString 으로 판별한다.

include::{snippets}/geometry-intersection-design-layer-land-linestring/curl-request.adoc[]
include::{snippets}/geometry-intersection-design-layer-land-linestring/http-response.adoc[]

[[resources-geometry-intersection-design-layer-land-polygon]]
=== polygon 입력에 대한 필지 정보

`GET` 시작 지점과 종료 지점이 같을 경우 polygon 으로 판별한다.

include::{snippets}/geometry-intersection-design-layer-land-polygon/curl-request.adoc[]
include::{snippets}/geometry-intersection-design-layer-land-polygon/http-response.adoc[]
...
include::{snippets}/geometry-intersection-data-linestring/http-response.adoc[]

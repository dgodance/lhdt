...
import lhdt.admin.svc.lhdt.domain.Layer;
import lhdt.admin.svc.lhdt.domain.LayerFileInfo;
import lhdt.admin.svc.lhdt.persistence.LayerFileInfoMapper;
import lhdt.admin.svc.lhdt.persistence.LayerMapper;
import lhdt.admin.svc.lhdt.service.GeoPolicyService;
import lhdt.admin.svc.lhdt.service.LayerService;
import lhdt.admin.svc.lhdt.support.LogMessageSupport;
...
//	@Value("${spring.datasource.password}")
    ...
    */
    ...
    public void insertOgr2Ogr(Layer layer, boolean isLayerFileInfoExist, String shapeFileName, String shapeEncoding) throws Exception {
		/*
		 * String osType = propertiesConfig.getOsType().toUpperCase(); String
		 * ogr2ogrPort = propertiesConfig.getOgr2ogrPort(); String ogr2ogrHost =
		 * propertiesConfig.getOgr2ogrHost(); String dbName = Crypt.decrypt(url); dbName
		 * = dbName.substring(dbName.lastIndexOf("/") + 1); String driver =
		 * "PG:host="+ogr2ogrHost + " port=" + ogr2ogrPort+ " dbname=" + dbName +
		 * " user=" + Crypt.decrypt(username) + " password=" + Crypt.decrypt(password);
		 * //Layer dbLayer = layerMapper.getLayer(layer.getLayerId());
		 * 
		 * String updateOption; if(isLayerFileInfoExist) { // update 실행 updateOption =
		 * "update"; } else { // insert 실행 updateOption = "insert"; }
		 * 
		 * GeoPolicy geoPolicy = geoPolicyService.getGeoPolicy(); String
		 * layerSourceCoordinate = layer.getCoordinate(); String layerTargetCoordinate =
		 * geoPolicy.getLayerTargetCoordinate(); // ShapeFileParser shapeFileParser =
		 * new ShapeFileParser(); // shapeFileParser.parse(shapeFileName); String
		 * enviromentPath = propertiesConfig.getOgr2ogrEnviromentPath(); Ogr2OgrExecute
		 * ogr2OgrExecute = new Ogr2OgrExecute(osType, driver, shapeFileName,
		 * shapeEncoding, layer.getLayerKey(), updateOption, layerSourceCoordinate,
		 * layerTargetCoordinate, enviromentPath); ogr2OgrExecute.insert();
		 */
    }
